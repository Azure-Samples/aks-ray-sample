...
# Default cluster config - can be overridden by command line args
num_workers = 10
cpus_per_worker = 12
block_size = 512
...
def main(args):
    ...
    # Get num_workers from args if provided, otherwise use default
    num_workers = args.num_workers if hasattr(args, 'num_workers') else num_workers

    # Reduce the dataset size to finish tuning in a reasonable time
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GPT-2 tuning script with Ray")
    parser.add_argument('--num_workers', type=int, default=10, help='Number of workers for distributed training')
